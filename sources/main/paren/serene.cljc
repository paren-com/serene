...
(defmulti transducer
  ...
  Returns a transducer that will modify spec entries during compilation."
...
#?(:clj (defn fetch
          ...
          ([url opts]
           (-> {:url url
                :method :post
                :body (json/write-str {:query introspection/query})}
             (merge opts)
             http/request
             deref
             :body
...
#?(:clj (defn spit-specs
          ([file ns introspection-response]
           (spit-specs file ns introspection-response nil))
          ([file ns introspection-response xform]
           (conform! simple-symbol? ns)
           (when (and
                   (-> file io/file .isFile)
                ...
                (doseq [form forms]
                  ...
                  (fipp/pprint
                    ...
                    {:symbols (assoc fipp/default-symbols
                                'clojure.spec.alpha/def (get fipp/default-symbols 'def))
                     :print-length nil
                     :print-level nil
                     :print-meta true
                     :width 100}))))))))
...
(defn prefix
  ...
  (let [ns (-> *ns* ns-name name)
        prefix-fn (if (simple-ident? prefix-fn)
...
(defn postfix-args
  ...
  (let [postfix-args-fn (if (simple-ident? postfix-args-fn)
                          (constantly postfix-args-fn)
                          postfix-args-fn)]
    (comp
      (map (partial
             walk/postwalk
